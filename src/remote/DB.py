...
class DB:
    ...
    def getHeader(self, type: str = "application/json"):
        ...
        return {"Authorization": f"Bearer {self.bearer}", "Content-Type": type}
    ...
    def status(self, response):
        if response.status_code == 200:
            ...
            if response.status_code == 401:
                raise HTTPException(
                    status_code=401, detail="Unauthorized - Please, update your bearer"
                )
    ...
    def patch(self, url: str, body: dict):
        ...
        response = requests.patch(
            self.makeUrl(url), json=body, headers=self.getHeader()
        )
        # return response.json()
        return self.status(response)

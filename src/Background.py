...
class Background:
    ...
    def __init__(self, back:List[str], task:TaskStatus|None=None, db:DB|None=None) -> None:
    ...
    def run(self):
        ...
        if self.taskStatus: self.taskStatus.sendRunning("running")
        ...
        if self.taskStatus: self.taskStatus.sendRunning("8 bit converted")
        ...
        if self.taskStatus: self.taskStatus.sendRunning("Background combiner done")
    ...
    def definePath(self):
        ...
        self.outputPath = Path(path_obj1.parent, f"{path_obj1.stem}_background_large_manual.tif" )
        # output_path = backurl
    ...
    def sendMediumBackground(self, instrumentId:str, projectId:str ):
        ...
        if ( self.taskStatus and self.db):
            ...
            url = self.db.makeUrl(f"/background/{instrumentId}/url?projectId={projectId}")
            print("url:",url)
    ...
    def returnData(self):
        ...
        if self.taskStatus: data["taskId"] = self.taskStatus.id

...
def test_hash_from_project():
    ...
    assert result == "dGVzdF9kcml2ZXx0ZXN0X3Byb2plY3Q="
...
def test_drive_and_project_from_hash_success(mock_get_drive_path):
    ...
    with patch.object(Path, "exists", return_value=True), patch.object(
        ...
        # Call the function
        drive_path, project_name = drive_and_project_from_hash(
            "dGVzdF9kcml2ZXx0ZXN0X3Byb2plY3Q="
        )
...
def test_hash_from_project_and_drive_and_project_from_hash_roundtrip(
    ...
    # Verify the hash is correctly formatted
    # assert project_hash == "test_drive|test_project"
...
def test_drive_and_project_from_hash_invalid_format(mock_get_drive_path):
    ...
    with pytest.raises(HTTPException) as excinfo:
        drive_and_project_from_hash("aW52YWxpZF9oYXNo")  # Base64 for "invalid_hash"
...
def test_drive_and_project_from_hash_drive_not_found(mock_get_drive_path):
    ...
    # Call the function
    with pytest.raises(HTTPException) as excinfo:
        drive_and_project_from_hash("dGVzdF9kcml2ZXx0ZXN0X3Byb2plY3Q=")

...
def test_save_and_load_matrix():
    ...
    save_matrix_as_gzip(test_matrix, test_file)

    # Load the matrix from the file
    loaded_matrix = load_matrix_from_gzip(test_file)
...
def test_different_sizes():
    ...
    for matrix, name in [
        ...
        save_matrix_as_gzip(matrix, test_file)
        loaded_matrix = load_matrix_from_gzip(test_file)
...
def test_load_matrix_from_compressed():
    """Test loading a matrix from compressed bytes content."""
    # Create a test matrix
    test_matrix = np.zeros((15, 25), dtype=bool)
    test_matrix[0, 0] = True
    test_matrix[7, 12] = True
    test_matrix[14, 24] = True

    # Save the matrix to a file
    test_file = "test_compressed_matrix.bin"
    save_matrix_as_gzip(test_matrix, test_file)
    ...
    # Load the matrix from the bytes content
    loaded_matrix = load_matrix_from_compressed(content)
    ...
    # Clean up
    os.remove(test_file)
    print("Compressed loading test passed!")


if __name__ == "__main__":
    test_save_and_load_matrix()
    test_different_sizes()
    test_load_matrix_from_compressed()
